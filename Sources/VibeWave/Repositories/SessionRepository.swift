...
public final class SessionRepository {
  ...
  public func createSessionsFromMessages() throws {
    try dbPool.write { db in
      ...
      try db.execute(sql: """
        ...
        SELECT
          m.session_id,
          MIN(m.created_at),
          (SELECT COALESCE(m_last.completed_at, m_last.created_at)
           FROM messages m_last
           WHERE m_last.session_id = m.session_id
  ...
  public func fetch(by sessionId: String) -> SessionRecord? {
    var result: SessionRecord?
    try? dbPool.read { db in
      if let row = try? Row.fetchOne(db, sql: "SELECT * FROM sessions WHERE session_id = ?", arguments: [sessionId]) {
        result = SessionRecord(row: row)
      }
    }
    return result
  }
  ...
  public func fetchAll() -> [SessionRecord] {
    ...
    try? dbPool.read { db in
      ...
      records = rows?.map { SessionRecord(row: $0) } ?? []
    }
    return records
  }

  // Fetch sessions by time range
  public func fetchByTimeRange(start: TimeInterval, end: TimeInterval) -> [SessionRecord] {
    var records: [SessionRecord] = []
    let startMs = Int64(start * 1000)
    let endMs = Int64(end * 1000)
    try? dbPool.read { db in
      let rows = try? Row.fetchAll(db, sql: "SELECT * FROM sessions WHERE first_message_at >= ? AND last_message_at <= ? ORDER BY last_message_at DESC", arguments: [startMs, endMs])
      records = rows?.map { SessionRecord(row: $0) } ?? []
    }
    return records
  }
  ...
  public func getSessionDepth(sessionId: String) -> String {
    guard let record = fetch(by: sessionId) else { return "unknown" }
    let userMsgCount = record.userMsgCount ?? 0
