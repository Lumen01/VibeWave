...
public enum SingleMetricValueStyle: Sendable {
    case compact
    case currency
...
public struct SingleMetricHistoryChartView: View {
    ...
    public init(
        ...
        self.yValueName = title
    }

    public var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            VStack(alignment: .leading, spacing: 0) {
                ChartHeaderView(title: title, mode: $mode)
    ...
    private var chartContent: some View {
        ...
        return VStack(spacing: 4) {
            ...
            statsView(plottedData: plottedData)
        }
    }

    private func statsView(plottedData: [SingleMetricDataPoint]) -> some View {
        ...
        return HStack(spacing: 16) {
            HStack(spacing: 4) {
                ...
                Text(valueStyle.format(maxValue))
            ...
            HStack(spacing: 4) {
                Text(L10n.chartAverage)
                    .font(.system(size: 10))
                    .foregroundColor(.secondary)
                Text(valueStyle.format(avgValue))
                    .font(.system(size: 10, weight: .medium))
            }
    ...
    private func chartView(layout: XAxisLayout, plottedData: [SingleMetricDataPoint]) -> some View {
...
func formatCostCompact(_ value: Double) -> String {
    let sign = value < 0 ? "-" : ""
    let absValue = abs(value)

    if absValue >= 1_000_000 {
        let d = absValue / 1_000_000.0
        let suffix = d.truncatingRemainder(dividingBy: 1) == 0
            ? String(format: "%.0fM", d)
            : String(format: "%.1fM", d)
        return "\(sign)$\(suffix)"
    }
