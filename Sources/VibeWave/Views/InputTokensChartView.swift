...
public struct InputTokensChartView: View {
    private let data: [InputTokensDataPoint]
    ...
    public var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            VStack(alignment: .leading, spacing: 0) {
                ChartHeaderView(title: "Input Tokens", mode: $mode)
    ...
    private var chartContent: some View {
        ...
        return ZStack(alignment: .topTrailing) {
            chartView(layout: layout, plottedData: plottedData)
    ...
    private func chartView(layout: XAxisLayout, plottedData: [InputTokensDataPoint]) -> some View {
    ...
    @ChartContentBuilder
    private func chartMarks(plottedData: [InputTokensDataPoint], layout: XAxisLayout) -> some ChartContent {
        if mode == .bar {
    ...
    private var chartData: [InputTokensDataPoint] {
    ...
    private func placeholderData(for timeRange: HistoryTimeRangeOption) -> [InputTokensDataPoint] {
        let now = Date()
        ...
        case .last24Hours:
            ...
            return TimeSeriesFiller.fillHourlyData(bucketValues: [:], startTime: window.start, barCount: 24)

        case .last30Days:
            let todayStart = calendar.startOfDay(for: now)
            let start = calendar.date(byAdding: .day, value: -29, to: todayStart) ?? todayStart
            return TimeSeriesFiller.fillDailyData(bucketValues: [:], startTime: start, barCount: 30)
        ...
        case .allTime:
            ...
            return TimeSeriesFiller.fillMonthlyData(bucketValues: [:], startTime: start, barCount: 12)
        }
    }

    private func yDomain(for plottedData: [InputTokensDataPoint]) -> ClosedRange<Double> {
        let maxValue = plottedData.map { Double($0.totalTokens) }.max() ?? 0
        if maxValue <= 0 {
            return 0...1
        }
    ...
    private func yGridValues(for plottedData: [InputTokensDataPoint]) -> [Double] {
    ...
    private func yLabelValues(for plottedData: [InputTokensDataPoint]) -> [Double] {
        let maxY = yDomain(for: plottedData).upperBound
    ...
    private func labeledBoundaryIndices(plottedData: [InputTokensDataPoint], layout: XAxisLayout) -> [Int] {
    ...
    private func xAxisLabel(
        boundaryIndex: Int,
        plottedData: [InputTokensDataPoint]
    ) -> String? {
    ...
    private func handleHover(
        ...
        case .active(let location):
            ...
            tooltipData = TooltipData(label: label, totalTokens: point.totalTokens)
    ...
    private func tooltipLabel(for point: InputTokensDataPoint) -> String {
        switch timeRange {
        case .last24Hours:
            return point.label
        case .last30Days:
    ...
    private func tooltipView(for data: TooltipData) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            HStack(spacing: 6) {
                Circle()
                    .fill(Constants.barColor)
                    .frame(width: 8, height: 8)

                Text(formatInputTokensCompact(Double(data.totalTokens)))
                    .font(.system(size: 11))
...
func formatInputTokensCompact(_ value: Double) -> String {
    if value >= 1_000_000 {
        let d = value / 1_000_000.0
